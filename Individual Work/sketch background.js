...
let easing = 0.00005; // Much slower easing for gradual animation
...
function draw() {
  ...
  // Draw multiple dynamic random circles at different positions
  noStroke();  // Remove the stroke for circles
  for (let i = 0; i < 5; i++) { // Draw 5 random circles
    // Randomize the position, size, and color for each circle
    let centerX = random(width);  // Random X position
    let centerY = random(height); // Random Y position
    let radius = random(30, 100); // Random radius for each circle

    // Dynamic color change based on time
    let r = map(sin(t + i), -1, 1, 100, 255); // Red component
    let g = map(cos(t + i), -1, 1, 100, 255); // Green component
    let b = map(sin(t + i + 1), -1, 1, 100, 255); // Blue component

    fill(r, g, b, 150); // Set the fill color with transparency
    ellipse(centerX, centerY, radius * 2, radius * 2); // Draw the circle

    // Make the circle size oscillate smoothly
    let scaleFactor = 1 + 0.5 * sin(t + i); // Smaller oscillation for smoother effect
    ellipse(centerX, centerY, radius * scaleFactor, radius * scaleFactor);
  }

  // Increment the time variable for smooth easing effect
  t += easing; // Slower speed by adjusting easing

  // Optionally reset `t` if it exceeds a certain limit (like 2PI or higher)
  if (t > TWO_PI) {
    t = 0;
  }
